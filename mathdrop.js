...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    setUp(setup_canvas, "white")


    class BaseSixtyNumber {
        constructor() {
            this.digit = this.sixtify(Math.floor(Math.random() * 60))
            this.x = Math.random() * (canvas.width - 50)
            this.y = Math.random() * -50
            let wet = 0
            for (let t = 0; t < nums.length; t++) {
                if (Math.abs(nums[t].x - this.x) < 50) {
                    wet = 1
                }
            }
            if (wet == 0) {
        ...
        draw() {
            ...
            canvas_context.drawImage(zimgs[this.digit], 0, 0, zimgs[this.digit].width, zimgs[this.digit].height, this.x, this.y, 50, 50)
            this.y += 49
    ...
    let nums = []
    let num = new BaseSixtyNumber()

    function main() {
        let num1 = new BaseSixtyNumber()
        if (Math.random() < .5) {
            let num2 = new BaseSixtyNumber()
        }

        // }
        canvas_context.fillStyle = `rgba(255,255,255, .25)`
